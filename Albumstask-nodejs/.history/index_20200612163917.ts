...
app.get('/getByTitleId', function (req: any, res: any) {
    ...
    con.query('select titledata.title_id, titledata.title_name,'
    +'artistsdata.artist_name,artistsdata.profession,'
    +'genre.genre_id,genre.genere_name, albumsdata.album_name,albumsdata.year,albumsdata.album_id'
   +'from titledata,artistsdata,genre,albumsdata'
  +'where(title_id,artist_id,genre_id,album_id)'
   +'=any (select title_id,artist_id,genre_id ,album_id from musicdata where title_id ='+title_id+')', (error: any, results: any) => {
            if (error) throw error;
            res.send(JSON.stringify(results));
        });
