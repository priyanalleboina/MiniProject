...
app.get('/getByAlbumId', function (req: any, res: any) {
    ...
    con.query('select album.titledata.title_id, album.titledata.title_name,artistsdata.artist_name,artistsdata.profession,genre.genre_id,genre.genere_namefrom titledata,artistsdata,genre where(title_id,artist_id,genre_id) =any (select title_id,artist_id,genre_id from musicdata where album_id = 1)',(error: any, results: any) => {
        if (error) throw error;
        res.send(JSON.stringify(results));
    });
