...
app.post('/musicalbumsDataCreate', function (req: any, res: any) {
    var params = {
        title_id: Number(req.body.title_id),
        album_id: Number(req.body.album_id),
        artist_id:Number(req.body.artist_id),
...
app.get('/getByAlbumId', function (req: any, res: any) {
    ...
    con.query('select  titledata.title_id, titledata.title_name,artistsdata.artist_name,artistsdata.profession,genre.genre_id,genre.genere_namefrom titledata,artistsdata,genre where(title_id,artist_id,genre_id) =any (select title_id,artist_id,genre_id from musicdata where album_id = 2)',(error: any, results: any) => {
        if (error) throw error;
        res.send(JSON.stringify(results));
    });
